...
import { CheckCircle2, XCircle, AlertTriangle, Minus, Sparkles, Loader2 } from 'lucide-react';
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-[#26221d]">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          {/* AWS Strands Agent Trigger Button */}
          {onInvestigateStrands && breakingCount > 0 && (
            <button
              type="button"
              onClick={onInvestigateStrands}
              disabled={isStrandsLoading}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-mono tracking-wide uppercase transition-all bg-[#ff6900]/10 hover:bg-[#ff6900]/20 text-[#ff6900] border border-[#ff6900]/30 hover:border-[#ff6900]/50 shadow-sm disabled:opacity-50"
            >
              {isStrandsLoading ? (
              ...
              <span>{isStrandsLoading ? 'Strands Active...' : 'Investigate (AWS Strands)'}</span>
            </button>
          )}
